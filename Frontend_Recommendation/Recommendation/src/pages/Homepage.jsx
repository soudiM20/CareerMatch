...
const Homepage = () => {
  ...
  // Fix (product-claim audit): this section previously rendered three
  // hardcoded fake internships (TechCorp India, StartupHub, FarmTech
  // Solutions) with a non-functional "Apply Now" button, styled to look
  // identical to real catalog data. GET /internships is a public,
  // unauthenticated endpoint that already returns the real active catalog
  // (see Backend/controllers/internshipController.js), so this now shows
  // actual open internships instead of invented ones.
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/internships`)
      .then((res) => { if (!cancelled) setFeaturedInternships(res.data.slice(0, 3)); })
  ...
  // Fix (product-claim audit): fields renamed to match the real Internship
  // document shape (Backend/models/Internship.js) now that this renders
  // actual catalog data instead of a hardcoded fake shape.
  const InternshipCard = ({ internship }) => (
    <div
      onClick={() => navigate(`/details/${internship.Internship_ID}`)}
    ...
    >
      <div className="p-6" style={{ padding: '24px' }}>
        ...
        <div className="flex items-center text-gray-600 mb-3" style={{ display: 'flex', alignItems: 'center', color: '#4b5563', marginBottom: '12px' }}>
          ...
          <span className="text-sm" style={{ fontSize: '14px' }}>{internship.Internship_District}</span>
          <Clock size={16} className="ml-4 mr-2 text-orange-500" style={{ marginLeft: '16px', marginRight: '8px', color: '#f97316' }} />
          <span className="text-sm" style={{ fontSize: '14px' }}>{internship.Duration_Months} months</span>
        </div>
        
        <div className="text-lg font-bold text-blue-600 mb-3" style={{ fontSize: '18px', fontWeight: 'bold', color: '#2563eb', marginBottom: '12px' }}>
          {internship.Stipend_Amount ? `₹${internship.Stipend_Amount}/month` : 'Unpaid'}
        </div>
        ...
        <button
          onClick={(e) => { e.stopPropagation(); navigate(`/details/${internship.Internship_ID}`); }}
    ...
    <div className="min-h-screen bg-gray-50" style={{ minHeight: '100vh', backgroundColor: '#f9fafb' }}>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center" style={{ maxWidth: '80rem', margin: '0 auto', padding: '0 32px', textAlign: 'center' }}>
          ...
          >
            Explore the current internship catalog and compare opportunities with your profile.
          </p>
          ...
          <div className="grid md:grid-cols-3 gap-8 mt-16" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '32px', marginTop: '64px' }}>
            <div className="text-center" style={{ textAlign: 'center' }}>
              ...
              {/* Fix (product-claim audit): was a fabricated "25.0K+" with
                  no data behind it. Nothing in this codebase counts total
                  internships served, so this is qualitative copy instead
                  of an invented number. */}
              <div className="text-gray-600 font-medium" style={{ color: '#4b5563', fontWeight: '500' }}>Real, active internships from the current catalog</div>
            </div>
            ...
            <div className="text-center" style={{ textAlign: 'center' }}>
              ...
              {/* Fix (product-claim audit): was a fabricated "500.0K+ Success
                  Stories" — this app has no user-outcome tracking, so there
                  is no real number to show here. */}
              <div className="text-gray-600 font-medium" style={{ color: '#4b5563', fontWeight: '500' }}>Explainable, skill-based matching</div>
            </div>
            ...
            <div className="text-center" style={{ textAlign: 'center' }}>
              ...
              {/* Fix (product-claim audit): was a fabricated "1.0K+ Partner
                  Companies" — there is no partner/company onboarding
                  concept anywhere in this codebase. */}
              <div className="text-gray-600 font-medium" style={{ color: '#4b5563', fontWeight: '500' }}>Opportunities across every listed sector</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-gray-50" style={{ padding: '64px 0', backgroundColor: '#f9fafb' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: '80rem', margin: '0 auto', padding: '0 32px' }}>
          ...
          {featuredError && (
            <p className="text-center text-gray-500">Couldn't load internships right now — please try again shortly.</p>
          )}
          {!featuredError && featuredInternships.length === 0 && (
            <p className="text-center text-gray-500">No open internships at the moment — check back soon.</p>
          )}
      ...
      <footer className="bg-gray-800 text-white py-12" style={{ backgroundColor: '#1f2937', color: 'white', padding: '48px 0' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: '80rem', margin: '0 auto', padding: '0 32px' }}>
          <div className="grid md:grid-cols-4 gap-8" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '32px' }}>
            <div>
              <div className="flex items-center space-x-3 mb-4" style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                ...
                <span className="text-xl font-bold" style={{ fontSize: '20px', fontWeight: 'bold' }}>CareerMatch</span>
              </div>
              <p className="text-gray-300" style={{ color: '#d1d5db' }}>
                A student-built tool for exploring explainable internship recommendations.
              </p>
            </div>
          ...
          {/* Fix (product-claim audit — the most serious footer finding):
              this claimed the app is the "PM Internship Scheme" — a real
              Government of India program this project has no affiliation
              with — and asserted copyright/official status it doesn't
              have. Left uncorrected, this reads as impersonating an
              official government initiative. Replaced with an accurate,
              unaffiliated line. */}
          <div className="border-t border-gray-700 mt-8 pt-8 text-center text-gray-300" style={{ borderTop: '1px solid #374151', marginTop: '32px', paddingTop: '32px', textAlign: 'center', color: '#d1d5db' }}>
            <p>&copy; 2026 CareerMatch. A student/demo project — not affiliated with any government initiative.</p>
          </div>
        </div>
      </footer>
    </div>
