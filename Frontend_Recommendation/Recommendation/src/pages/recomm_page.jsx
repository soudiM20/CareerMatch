...
export default function RecommendationPage() {
  ...
  const [filters, setFilters] = useState({ role: "", location: "", workMode: "", skills: "", minMatchScore: "" });
  ...
  const load = async (event) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${API}/internships/recommend`, { params, headers: { Authorization: `Bearer ${token}` } });
      setItems(data);
    } catch (requestError) {
      setError(requestError.response?.data?.message || "Recommendations could not be loaded. Complete your profile and try again.");
    } finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);

  return <main className="min-h-screen bg-slate-50 px-4 py-10"><div className="mx-auto max-w-7xl">
    ...
    <form onSubmit={load} className="mb-8 grid gap-3 rounded-2xl bg-white p-4 shadow-sm md:grid-cols-6"><label className="md:col-span-2"><span className="sr-only">Role</span><input value={filters.role} onChange={e => setFilters({...filters, role:e.target.value})} placeholder="Role or sector" className="w-full rounded-lg border border-slate-200 px-3 py-2" /></label><input value={filters.location} onChange={e => setFilters({...filters, location:e.target.value})} placeholder="Location" className="rounded-lg border border-slate-200 px-3 py-2" /><select value={filters.workMode} onChange={e => setFilters({...filters, workMode:e.target.value})} className="rounded-lg border border-slate-200 px-3 py-2"><option value="">Any work mode</option><option>Remote</option><option>Hybrid</option><option>On-site</option></select><input value={filters.minMatchScore} onChange={e => setFilters({...filters, minMatchScore:e.target.value})} type="number" min="0" max="100" placeholder="Min. score" className="rounded-lg border border-slate-200 px-3 py-2" /><button className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700"><Filter size={16}/>Filter</button></form>
